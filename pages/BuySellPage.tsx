...
const ItemCard: React.FC<{
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden flex flex-col">
        ...
        <div className="p-2 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center gap-2">
            {item.status === 'pending' && (
                <>
                    <button onClick={() => onApprove(item.id)} className="flex-1 text-sm flex items-center justify-center gap-2 bg-green-100 text-green-700 hover:bg-green-200 p-2 rounded-md"><CheckCircleIcon className="w-5 h-5"/> موافقة</button>
                    <button onClick={() => onReject(item.id)} className="flex-1 text-sm flex items-center justify-center gap-2 bg-orange-100 text-orange-700 hover:bg-orange-200 p-2 rounded-md"><XCircleIcon className="w-5 h-5"/> رفض</button>
                </>
            )}
            <button onClick={() => onDelete(item.id)} className="flex-1 text-sm flex items-center justify-center gap-2 bg-red-100 text-red-700 hover:bg-red-200 p-2 rounded-md"><TrashIcon className="w-5 h-5"/> حذف</button>
        </div>
    </div>
);
