...
import { getPublicContent, getAppLinks } from '../api/generalApi';
...
const PublicHomePage: React.FC = () => {
    useDocumentTitle('الرئيسية | Helio');
    const contentQuery = useQuery({ queryKey: ['publicContent'], queryFn: getPublicContent });
    const linksQuery = useQuery({ queryKey: ['appLinks'], queryFn: getAppLinks });

    const content = contentQuery.data?.home;
    const googlePlayUrl = linksQuery.data?.googlePlayUrl || '#';
    const appleAppStoreUrl = linksQuery.data?.appleAppStoreUrl || '';
    ...
    if (!content) return null; // Or a loading spinner handled by wrapper

    return (
        <QueryStateWrapper queries={[contentQuery, linksQuery]}>
